...
export class FileUploadGenericComponent implements OnInit, OnDestroy {
  ...
  @Input() btnClass   = "btn btn-link font-weight-bold pt-0 pb-0"
  @Input() uplaodPath = "uploadIBNR"
  ...
  constructor(private fs: FileService, private tsr:ToasterService) {
    this.uploaderId = FileUploadGenericComponent.UPLOADER_ID++;
  }
  ...
  onFileChange(event) {
    ...
    if (!allowedExtensions.exec(myFile.name)) {
      ...
      //this.tsr.pop("warning","Warning",)

      this.fileUploadFailed.emit("The file has not been uploaded. IBNR file supported format is csv.")
