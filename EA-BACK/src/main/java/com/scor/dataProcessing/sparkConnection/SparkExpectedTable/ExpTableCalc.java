...
//import java.util.*;
//
//public class ExpTableCalc  implements Serializable {
//    /**
//     *
//     */
//    private static final long serialVersionUID = 1L;
//
//    public static HashMap<String,Long> run(String path, String type){
...
//       {
//           JavaRDD<Long> rdd = data_splitted.map(s-> !(s.length <= names.indexOf("insurance_age_attained")) && s[names.indexOf("insurance_age_attained")].matches("^-?\\d{1,19}$") ? Long.parseLong(s[names.indexOf("insurance_age_attained")]):null).filter(s->s != null);
//           rdd.cache();
//           max_age =rdd.max(cmp);
//           min_age = rdd.min(cmp);
//           rdd.unpersist();
...
//       {
//           JavaRDD<Long> rdd = data_splitted.map(s-> !(s.length <= names.indexOf("age_attained")) && s[names.indexOf("age_attained")].matches("^-?\\d{1,19}$") ? Long.parseLong(s[names.indexOf("age_attained")]):null).filter(s->s != null);
...
//        {
//            JavaRDD<Long> rdd = data_splitted.map(s-> !(s.length <= names.indexOf("age_at_commencement")) && s[names.indexOf("age_at_commencement")].matches("^-?\\d{1,19}$") ? Long.parseLong(s[names.indexOf("age_at_commencement")]):null).filter(s->s != null);
//            HashMap<String,Long> MinMax = FindMinAndMax(rdd,cmp);
//            if(MinMax != null) {
//                min_age = MinMax.get("min");
//                max_age = MinMax.get("max");
...
//       {
//           JavaRDD<Long> rdd = data_splitted.map(s-> !(s.length <= names.indexOf("duration_year")) && s[names.indexOf("duration_year")].matches("^-?\\d{1,19}$") ? Long.parseLong(s[names.indexOf("duration_year")]):null).filter(s->s != null);
//           HashMap<String,Long> MinMax = FindMinAndMax(rdd,cmp);
//           if(MinMax != null) {
...
//       {
//           JavaRDD<Long> rdd = data_splitted.map(s->  !(s.length <= names.indexOf("calendar_year")) &&  s[names.indexOf("calendar_year")].matches("^-?\\d{1,19}$") ? Long.parseLong(s[names.indexOf("calendar_year")]):null).filter(s->s != null);
//           HashMap<String,Long> MinMax = FindMinAndMax(rdd,cmp);
//           if(MinMax != null) {
